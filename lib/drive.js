...
export async function uploadImageFromUrl({ title, imageUrl, contentType, campaign, agent }) {
  ...
  // Download/decode the image (supports both HTTP URLs and base64 data URLs)
  let imageBuffer
  let imageMime
  if (imageUrl.startsWith('data:')) {
    ...
    imageMime = imageRes.headers.get('content-type') || 'image/png'
  }
  ...
  const metadata = {
    name: `${title}${ext}`,
    parents: [folderId],
    description: `Generated by ${agent || 'PIXEL'} via FLUX.1 | Campaign: ${campaign || 'N/A'}`,
  }
...
export async function exportToDrive(task) {
  ...
  try {
    ...
    if (contentType === 'Image' && output) {
      // Extract image URL from output (HTTP URLs or base64 data URLs)
