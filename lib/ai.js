...
// Tracks API calls against daily budget cap

import { checkBudget, trackApiCall } from './system-config'
 ...
 */
export async function callAI({ model, temperature, systemPrompt, userPrompt, maxRetries = 2, skipBudgetCheck = false }) {
  // ── Budget gate: check before making any API call ──
  if (!skipBudgetCheck) {
    try {
      const budget = await checkBudget()
      if (!budget.allowed) {
        const reason = budget.autoPaused
          ? `Daily budget of ${budget.limit} API calls reached. System has been auto-paused.`
          : `System is paused or over budget (${budget.used}/${budget.limit} calls used today).`
        throw new Error(`BUDGET_EXCEEDED: ${reason}`)
      }
    } catch (err) {
      if (err.message?.startsWith('BUDGET_EXCEEDED:')) throw err
      // If budget check itself fails, allow the call (fail-open for budget check only)
      console.warn('[AI] Budget check failed, allowing call:', err.message)
    }
  }
  ...
  for (let pi = 0; pi < providerChain.length; pi++) {
    ...
    try {
      ...
      // ── Track successful call against budget ──
      try {
        await trackApiCall()
      } catch (trackErr) {
        console.warn('[AI] Failed to track API call:', trackErr.message)
      }
