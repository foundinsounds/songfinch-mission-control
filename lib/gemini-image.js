...
export async function generateImage({
  ...
  // Dedicated image-gen variant with responseModalities: ["IMAGE", "TEXT"]
  ...
  if (!res.ok) {
    ...
    console.warn(`[GEMINI-IMG] Flash image gen failed (${res.status}), trying Imagen 4...`)
  ...
  // If no image in response, fall back to Imagen 4
  console.warn(`[GEMINI-IMG] Flash response had no image, trying Imagen 4...`)
  return generateWithImagen3({ prompt: enhancedPrompt, style, preset, territory })
}

/**
 * Fallback: Generate with Imagen 4 via the dedicated predict endpoint.
 */
async function generateWithImagen3({ prompt, style, preset, territory }) {
  const url = `https://generativelanguage.googleapis.com/v1beta/models/imagen-4.0-generate-001:predict?key=${GOOGLE_AI_KEY}`
  ...
  if (!res.ok) {
    const error = await res.text()
    throw new Error(`Imagen 4 API error: ${res.status} - ${error}`)
  }
